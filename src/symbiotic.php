<?php
namespace Symbiotic\Core\Support { use ArrayAccess, InvalidArgumentException, Traversable, ArrayIterator; use Symbiotic\Container\{ArrayAccessTrait, MagicAccessTrait}; use function _DS\{value, data_get}; trait CollectionTrait { use ArrayAccessTrait, MagicAccessTrait; protected $items = []; function __construct($items = []){
 ...
 namespace Symbiotic\Http\Middleware { use Psr\Http\Message\{ResponseInterface, ServerRequestInterface}; use Psr\Http\Server\{MiddlewareInterface, RequestHandlerInterface}; use Symbiotic\Http\UriHelper; trait MiddlewaresCollectionTrait { protected $middleware = []; function append(MiddlewareInterface $middleware){
 ...
 } namespace Symbiotic\Storage { interface RememberingInterface { function remember(string $key, callable $value);}}
 namespace Symbiotic\Container { use ReflectionMethod, ReflectionFunction, InvalidArgumentException, Closure, Psr\SimpleCache\CacheInterface, Throwable; use Psr\Container\{ContainerExceptionInterface, ContainerInterface, NotFoundExceptionInterface}; trait ArrayAccessTrait { function offsetExists($key){
 ...
 $this->app = $container;}}
 } namespace Symbiotic\Core { use Symbiotic\Container\{ CachedContainerInterface, CachedContainerTrait, DIContainerInterface, ArrayAccessTrait, SingletonTrait, Container, ServiceContainerTrait, ServiceContainerInterface }; use Symbiotic\Core\Bootstrap\{ BootBootstrap, CoreBootstrap, ProvidersBootstrap, LazyPackagesBootstrap, }; use Symbiotic\Core\Support\Collection, Psr\SimpleCache\CacheInterface, Psr\Http\Message\ResponseInterface, Psr\Http\Server\RequestHandlerInterface, Symbiotic\Packages\PackagesRepositoryInterface; interface BootstrapInterface { function bootstrap(CoreInterface $app): void; } interface RunnerInterface { function isHandle(): bool; function run(): void; } interface ServiceProviderInterface { function register(): void; function boot(): void; function bindings(): array; function singletons(): array; function aliases(): array; } interface HttpKernelInterface extends RequestHandlerInterface { function bootstrap(): void; function response(int $code = 200, \Throwable $exception = null): ResponseInterface; } interface CoreInterface extends DIContainerInterface, ServiceContainerInterface { function addBootstraps($bootstraps); function isBooted(): bool; function bootstrap(): void; function runBootstrap($class): void; function addRunner(RunnerInterface $runner): void; function run(): void; } class Autoloader { protected static $namespace = 'Symbiotic'; private static $registered = false; protected static $packages_dirs = []; protected static $strorage_path = null; protected static $registered_namespaces = []; protected static $files = []; protected static $classes = []; static function register($prepend = false, array $scan_dirs = null,$storage_path = null){
 if ($storage_path){
 ...
 } namespace Symbiotic\Filesystem { use Symbiotic\Container\DIContainerInterface, Symbiotic\Filesystem\Adapter\Local; trait ArrayStorageTrait { private $storage_path = null; protected function setStoragePath(string $path){
 ...
 } namespace Symbiotic\Core\View { use Symbiotic\{ Core\Support\RenderableInterface, Core\CoreInterface, Core\Support\Str, Apps\ApplicationInterface, Packages\TemplatesRepositoryInterface }; class Section { public $sections = array(); public $last = array(); function start($section,$content = null){
 if ($content === null){
 ...
 $app[CacheInterface::class]->clear(); }); $app->alias(CacheInterface::class, \Symbiotic\SimpleCacheFilesystem\SimpleCacheInterface::class);}}
 ...
 } namespace Symbiotic\Session { use Symbiotic\Core\ServiceProvider; use Symbiotic\Container\{ArrayContainerInterface, ArrayAccessTrait}; interface SessionStorageInterface extends ArrayContainerInterface { function start(); function getId(); function setId(string $id); function getName(); function save(); function clear(); function isStarted(); } class NativeProvider extends ServiceProvider { function register(): void { $this->app->singleton(SessionStorageInterface::class, function ($app){
 ...
 } namespace Symbiotic\Http\Kernel { use Symbiotic\Core\{ CoreInterface, BootstrapInterface, HttpKernelInterface, View\View, Runner, Support\ArrayableInterface, Support\RenderableInterface }; use Symbiotic\Http\Middleware\{ MiddlewaresCollection, MiddlewaresDispatcher, MiddlewareCallback, MiddlewaresHandler, RequestPrefixMiddleware }; use Symbiotic\Http\{ ResponseSender, PsrHttpFactory, UriHelper, ResponseMutable }; use Symbiotic\{ Packages\AssetFileMiddleware, Apps\ApplicationInterface, Apps\AppsRepositoryInterface, Routing\RouteInterface }; use Psr\Http\Message\{ ResponseFactoryInterface, ServerRequestInterface, ResponseInterface, StreamInterface }; use Psr\Http\Server\RequestHandlerInterface; use function _DS\config; class Bootstrap implements BootstrapInterface { function bootstrap(CoreInterface $app): void { $app->singleton(HttpKernelInterface::class, HttpKernel::class); $app->addRunner(new HttpRunner($app));}}
 ...
 } namespace Symbiotic\Core\Bootstrap { use Symbiotic\Core\{ BootstrapInterface, CoreInterface, Config, Events\CacheClear, View\View, AbstractBootstrap, ProvidersRepository }; use Symbiotic\Event\{ EventDispatcher, ListenerProvider, ListenersInterface }; use Psr\EventDispatcher\{ListenerProviderInterface, EventDispatcherInterface}; use Symbiotic\Packages\{LazyPackagesDecorator, PackagesRepositoryInterface}; use function _DS\config; class BootBootstrap { function bootstrap($app){
 $app->boot();}}
 ...
 } namespace Symbiotic\View\Blade { use Symbiotic\Packages\{TemplateCompilerInterface, TemplateCompiler}; use Symbiotic\Core\CoreInterface; class Bootstrap implements \Symbiotic\Core\BootstrapInterface { function bootstrap(CoreInterface $app): void { $app->afterResolving(TemplateCompiler::class, function (TemplateCompiler $compiler){
 ...
 $pattern = $this->matcher('include');return preg_replace($pattern, '$1<?php echo \\Symbiotic\\Core\\View\\View::make$2->with(get_defined_vars())->render(); ?>',$value);}protected function compile_render($value){
 ...
 if (\is_readable($file)){
 $config = Arr::get(@\json_decode(file_get_contents($file), true), 'extra.symbiotic');if(is_array($config)){
 ...
 const A = 'application/'; const I = 'image/'; const T = 'text/'; } namespace Symbiotic\Filesystem\Adapter { use Symbiotic\Filesystem\{ AdapterInterface, PathPrefixInterface, FilesystemInterface }; abstract class AbstractAdapter implements AdapterInterface, PathPrefixInterface { protected $path_prefix = '/'; function setPathPrefix($path){
 if (!empty($path)){
 ...
 } } } namespace _DS { use Symbiotic\Core\{ Config, Core, Support\Str, Support\Collection, Support\Arr, HttpKernelInterface }; use Psr\Http\Message\{ResponseFactoryInterface, ResponseInterface}; const DS = DIRECTORY_SEPARATOR; function app($abstract = null, array $parameters = null){
 ...
 } namespace Symbiotic\Core\Events { class CacheClear { protected $path = null; function __construct(string $path = null){
